...
(function () {
  ...
  var SECTIONS = [
    ...
    { id: "cible", icon: "ti-target", title: "Cible", req: true,
      ...
      body: function () { return '' +
        ...
        '<label class="lbl">Segment (secteur) <span class="mini">(optionnel, libre ; clique × pour retirer)</span></label><div class="row">' + tags("segments") + '</div>' +
  ...
  function submit() {
    ...
    L.push("");
    L.push("Étape 2 — CAMPAGNE : campaign_create (name + angle + persona + steps selon la cadence — chaque étape porte son canal (email/linkedin) + son action + son délai, lead_ids). La maquette héritera de cette séquence ; le contenu de chaque étape (email ou message/action LinkedIn) y sera rédigé." + (S.baseModelId ? " Passe base_model_id." : (S.templateId ? " Repars du template." : "")) + (S.segments.length ? (" Passe industries=[" + S.segments.map(function(s){return '"'+s+'"';}).join(", ") + "].") : ""));
    L.push("Étape 3 — set_campaign_config sur le DRAFT : objective, angle, persona, icp_description" + (S.segments.length ? (", industries=[" + S.segments.map(function(s){return '"'+s+'"';}).join(", ") + "]") : "") + (S.signals.length ? (", signal_type_key=" + S.signals[0]) : "") + (S.channel !== "email" ? (", channel_mode=" + S.channel + " (séquence " + presetName() + ")") : "") + (S.voiceText.trim() ? ", voice_description (override)" : "") + ".");
