(function(){
      ...
      function buildFinalTable(){
        return aliveCards().map(function(card, i){
          var s = readCard(card);
          var flags = (s.regen ? " [RÉGÉNÉRER]" : "") + (s.isNew ? " [NOUVELLE]" : "");
      ...
      function commitPrompt(mode){
        ...
        p += "2) Étapes marquées [RÉGÉNÉRER]" + (mode === "relaunch" ? " ET toutes les étapes (relance complète)" : "") + " → appelle message_generate" + leadRef + " pour réécrire le corps on-voice selon l'angle, puis re-persiste.\n";
      ...
      cardsWrap.addEventListener("click", function(e){
        ...
        if (e.target.matches(".angtog")) { var aw = card.querySelector(".angwrap"); var open = aw.classList.toggle("open"); e.target.setAttribute("aria-expanded", open); return; }
        if (e.target.matches(".btn-regen")) { var on = card.classList.toggle("regen"); e.target.setAttribute("aria-pressed", on); recompute(); return; }
        if (e.target.matches(".btn-insert")) { insertBefore(card); return; }
        if (e.target.matches(".ins-cta")) {
          var bo = card.querySelector(".f-body"); if (!bo) return;
          var p = bo.selectionStart != null ? bo.selectionStart : bo.value.length, ins = "[Réserver 30 minutes](booking)";
          bo.value = bo.value.slice(0, p) + ins + bo.value.slice(bo.selectionEnd != null ? bo.selectionEnd : p);
          recompute(); return;
        }
