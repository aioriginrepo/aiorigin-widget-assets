(function(){
      ...
      function lintBody(body){
        ...
        var anchors = (body.match(/\]\(booking\)/g) || []).length;
        if (brackets > anchors) o.push(["err", "Crochet [ non autorisé (seul [..](booking) l'est)"]);
        else if (hasBooking) o.push(["ok", "CTA booking OK"]);
        if (/https?:\/\//i.test(body)) o.push(["err", "URL brute (utilise [texte](booking))"]);
      ...
      function cardErrors(card){
        ...
        var isEmail = (card.getAttribute("data-channel") || "email") === "email";
        var e = 0, all = lintBody(fval(card, ".f-body")).concat(lintSubject(fval(card, ".f-subject"), isEmail));
      ...
      function paintCard(card, sendDay){
        ...
        var subEl = card.querySelector(".smsub"); if (subEl) subEl.textContent = fval(card, ".f-subject") || "(objet vide)";
        ...
        if (meta) meta.textContent = (del ? "supprimée" : (sendDay === 0 ? "Envoi immédiat (J0)" : "Envoi J+" + sendDay)) + (first ? "  ·  " + first.slice(0, 80) : "");

        var lb = card.querySelector(".lintbox");
        if (lb) {
          var lc = del ? [] : lintBody(body), html = "";
        ...
        var cta = card.querySelector(".f-cta"); if (cta && document.activeElement !== cta) cta.checked = body.indexOf("](booking)") >= 0;
      ...
      function buildFinalTable(){
        return aliveCards().map(function(card, i){
          ...
          var flags = (s.regen ? (note ? " [RÉGÉNÉRER selon consigne: " + note + "]" : " [RÉGÉNÉRER]") : "") + (s.isNew ? " [NOUVELLE]" : "");
          var line = (i+1) + ") role=" + (s.role || "?") + ", delay_days=" + s.delay + ", subject=\"" + s.subject.replace(/"/g, "'") + "\"" + flags + "\n   angle: " + s.angle;
          if (!s.regen && (s.isNew || (card._base && card._base.body !== s.body))) {
            line += "\n   corps:\n\"\"\"\n" + s.body + "\n\"\"\"";
          }
          return line;
        }).join("\n");
      ...
      function commitPrompt(mode){
        ...
        p += "2) Étapes marquées [RÉGÉNÉRER...]" + (mode === "relaunch" ? " ET toutes les étapes (relance complète)" : "") + " → appelle message_generate" + leadRef + " pour réécrire le corps on-voice selon l'angle ET la consigne entre crochets si présente (« selon consigne: … »), puis re-persiste.\n";
        p += "3) Étapes [NOUVELLE] → rédige un corps complet on-voice grounded sur les faits du lead (objet + corps, CTA via [..](booking)).\n";
      ...
      cardsWrap.addEventListener("change", function(e){
        if (e.target.matches(".f-role")) recompute();
          ...
          if (e.target.checked && !has) bo.value = bo.value.replace(/\s*$/, "") + "\n\n[Réserver 30 minutes](booking)";
          else if (!e.target.checked && has) bo.value = bo.value.replace(/\n*[ \t]*\[[^\]]*\]\(booking\)/g, "");
          bo.style.height = "auto"; bo.style.height = bo.scrollHeight + "px"; recompute();
        }
      ...
      cardsWrap.addEventListener("click", function(e){
        ...
        if (e.target.matches(".btn-regen")) { var on = card.classList.toggle("regen"); e.target.setAttribute("aria-pressed", on); var rn = card.querySelector(".regen-note"); if (rn) rn.style.display = on ? "block" : "none"; recompute(); return; }
      ...
      if (chanSel) chanSel.addEventListener("change", function(){ if (chanSel.value !== "email") { chanSel.value = "email"; return; } if (policyScope) send("Mets le mode canal de " + pbRef + " sur email via campaign_policy_set({campaign_id=" + (CTX.campaign_id||"") + ", channel_mode=email}), puis re-render sequence_show."); });
